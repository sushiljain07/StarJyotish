...
export default function LandingStickyHeader({ visible, onLanguageChange, currentLanguage, onCtaClick }) {
  ...
  return (
    <div
      className={`fixed top-0 inset-x-0 z-30 bg-parchment-card/95 backdrop-blur border-b border-line shadow-sm transition-transform duration-300 ${
    ...
    >
      <div className="max-w-4xl mx-auto px-4 py-2.5 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="font-bold text-ink text-sm truncate">{t('app_title')}</span>
        </div>
              ...
              <button
                ...
                className={`px-2 py-1 rounded-full text-[11px] font-semibold transition ${
                  currentLanguage.startsWith(lang)
                    ...
                    : 'bg-night/5 text-ink-muted hover:bg-night/10'
                }`}
              >
                {lang === 'en' ? 'EN' : 'हि'}
              </button>
