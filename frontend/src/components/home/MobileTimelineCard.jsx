...
//
// Mobile-first replacement for the SkyRhythm SVG arc.
// The arc is beautiful on desktop but unreadable on 375px screens — window
// labels overlap, text shrinks below legibility, and the NOW marker is a
// tiny dot in a sea of noise.
//
// This card answers the same question in a format that scales to any screen:
//   Zone 1  —  Current Status Card   "Is now a good time?"
//   Zone 2  —  Compact Bar Timeline  Visual day overview (sunrise → sunset)
//   Zone 3  —  Today's Windows       All muhurta slots, tapable for detail
//   Zone 4  —  AI Insight strip      One-line contextual guidance
//
// Data contract: same `panchang` prop as SkyRhythm — no extra API calls.
// The window detail logic is lifted directly from DailyTimeline so both
// views stay in sync without duplicating the SLOT_DETAILS table.
...
const SLOT_DETAILS = {
  ...
  gulika:    { do: ['Routine administrative work', 'Desk work and correspondence'], avoid: ['Starting new projects', 'Financial commitments', 'Beginning travel'] },
  yamaganda: { do: ['Reading, study, inner work'], avoid: ['Signing documents', 'New negotiations', 'Medical procedures'] },
  abhijit:   { do: ['Financial conversations', 'Interviews and negotiations', 'Starting anything important'], avoid: ['Emotional confrontations'] },
  rahu:      { do: ['Routine tasks', 'Exercise', 'Reading'], avoid: ['Signing contracts', 'New investments', 'New purchases', 'Starting travel'] },
  sunset:    { do: ['Evening prayers', 'Reflection and journaling', 'Family time'], avoid: [] },
}
...
function currentSlot(slots, nowMin) {
  return slots.find(s => {
    if (s.kind === 'anchor') return Math.abs(nowMin - s.start) < 5
    return s.end != null && nowMin >= s.start && nowMin < s.end
  }) ?? null
}

function nextSlot(slots, nowMin) {
  return slots.find(s => s.kind !== 'anchor' && s.start > nowMin) ?? null
}

// ─── Zone 1: Current Status Card ─────────────────────────────────────────────

const STATUS = {
  good:   { bg: '#1A3B2F', border: '#2E7D5E', dot: '#4CAF7D', textColor: '#7FCFA0', label: 'status_favorable', icon: '✓' },
  avoid:  { bg: '#3B1A1A', border: '#8B3A3A', dot: '#E05555', textColor: '#E08080', label: 'status_avoid',     icon: '✗' },
  anchor: { bg: '#2A2410', border: '#9A7B2A', dot: '#D9A441', textColor: '#D9A441', label: 'status_neutral',   icon: '◎' },
  none:   { bg: '#1E2240', border: '#3A4070', dot: '#6A7CC0', textColor: '#9AA8D8', label: 'status_neutral',   icon: '◎' },
}

function StatusCard({ slot, nowMin, slots, t }) {
  const cfg = slot ? STATUS[slot.kind] ?? STATUS.none : STATUS.none
  const next = nextSlot(slots, nowMin)
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
        <div>
          <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: cfg.textColor, marginBottom: 2 }}>
            {t('status_current_time')}
          </p>
          <p style={{ fontSize: 22, fontWeight: 700, color: cfg.textColor, lineHeight: 1.15, fontFamily: 'Fraunces, Georgia, serif' }}>
            {t(cfg.label)}
          </p>
          {slot && (
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
            <span style={{
              ...
              background: next.kind === 'good' ? '#4CAF7D' : next.kind === 'avoid' ? '#E05555' : '#D9A441',
              flexShrink: 0,
            }} />
            <div>
...
// ─── Zone 2: Compact Bar Timeline ────────────────────────────────────────────

function BarTimeline({ slots, nowMin, sunriseMin, sunsetMin }) {
  const START = sunriseMin ?? 360   // 6 AM fallback
  const END   = sunsetMin  ?? 1110  // 6:30 PM fallback
  ...
  const segments = []
  let cursor = START
  const windowSlots = slots.filter(s => s.kind !== 'anchor' && s.start != null && s.end != null)

  // Sort and build segments
  const sorted = [...windowSlots].sort((a, b) => a.start - b.start)
  for (const s of sorted) {
    const segStart = Math.max(cursor, START)
    const segEnd   = Math.min(s.start, END)
    if (segEnd > segStart) {
      segments.push({ start: segStart, end: segEnd, kind: 'neutral' })
    }
    if (s.start < END) {
  ...
  const COLOR = { avoid: '#C0392B', good: '#2E7D5E', neutral: '#3D4466' }
  ...
  return (
    <div style={{ marginBottom: 10 }}>
      {/* Bar */}
      <div style={{ position: 'relative', height: 10, borderRadius: 10, overflow: 'hidden', display: 'flex', background: '#3D4466' }}>
        {segments.map((seg, i) => (
          <div key={i} style={{
            position: 'absolute',
            left: `${pct(seg.start)}%`,
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 5 }}>
        <span style={{ fontSize: 10, color: 'rgba(248,242,228,0.4)', fontWeight: 600 }}>
          ☀ {fmt(sunriseMin)}
        </span>
        {showNow && (
          <span style={{
            ...
            transform: `translateX(${nowPct - 50}%)`,
            position: 'absolute',
            left: `${nowPct}%`,
            marginLeft: -18,
          }}>
      ...
      <div style={{ display: 'flex', gap: 12, marginTop: 6 }}>
        {[
          ...
          { color: '#2E7D5E', label: 'Favorable' },
          { color: '#3D4466', label: 'Neutral' },
        ].map(({ color, label }) => (
    ...
    <div>
      <button
        ...
        style={{
          ...
          padding: '10px 12px',
          borderRadius: 10,
          border: isNow ? '1px solid rgba(217,164,65,0.35)' : '1px solid transparent',
          background: isNow ? 'rgba(217,164,65,0.06)' : open ? 'rgba(255,255,255,0.04)' : 'transparent',
          cursor: 'pointer',
          textAlign: 'left',
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <span style={{ fontSize: 13, fontWeight: 600, color: 'rgba(248,242,228,0.9)' }}>
              {t(slot.labelKey)}
            </span>
            {isNow && (
              <span style={{
                fontSize: 8, fontWeight: 800, letterSpacing: '0.08em',
                color: '#D9A441', background: 'rgba(217,164,65,0.15)',
        ...
        {details && (
          <span style={{
            fontSize: 10, color: 'rgba(248,242,228,0.3)',
      ...
      {open && details && (
        <div style={{
          ...
          <div>
            <p style={{ fontSize: 9, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#7FCFA0', marginBottom: 6 }}>
              {t('timeline_good_for')}
            </p>
          ...
          <div>
            <p style={{ fontSize: 9, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#E08080', marginBottom: 6 }}>
              {t('timeline_avoid')}
            </p>
...
export default function MobileTimelineCard({ panchang }) {
  ...
  const active   = currentSlot(slots, nowMin)
  const nextGood = slots.find(s => s.kind === 'good' && s.start > nowMin) ?? null
  ...
  return (
    <div style={{
      ...
      {/* Zone 3 — Today's Windows */}
      <p style={{
        fontSize: 10, fontWeight: 700, textTransform: 'uppercase',
        letterSpacing: '0.08em', color: 'rgba(248,242,228,0.35)',
