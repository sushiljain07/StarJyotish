...
import * as C from '../../../config/marriageMangalDoshaContent'
...
export default function MangalDoshaGuide() {
  const { isAuthenticated } = useAuth()
  const sections = Object.entries(C).filter(([k]) => !['HERO','QUICK_FACTS','FAQ','CTA'].includes(k))
  
  return (
    <KnowledgeLayout
      seoTitle="Mangal Dosha — Marriage & Compatibility Path"
      seoDescription={C.HERO.subtitle}
      path="/learn/marriage/mangal-dosha"
      eyebrow="Marriage & Compatibility · Part 5 of 7"
      title={C.HERO.title}
      subtitle={C.HERO.subtitle}
      meta={<LearningMetadata estimatedReadTime={8} difficulty="intermediate" category="Marriage" lastUpdated="2026-07-18" />}
      breadcrumbItems={[
    ...
    >
      ...
      <div className="mt-10">
        <RelatedArticles variant="next" title="Marriage & Compatibility path" items={[
          { title: 'Marriage Timing: When Dashas Point to Partnership', href: '/learn/marriage/marriage-timing', comingSoon: false, description: 'Continue the path.' },
        ]} />
      </div>
    </KnowledgeLayout>
  )
