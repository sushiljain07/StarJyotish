...
import * as C from '../../../config/marriageTimingContent'
...
export default function MarriageTimingGuide() {
  const { isAuthenticated } = useAuth()
  const sections = Object.entries(C).filter(([k]) => !['HERO','QUICK_FACTS','FAQ','CTA'].includes(k))
  
  return (
    <KnowledgeLayout
      seoTitle="Marriage Timing — Marriage & Compatibility Path"
      seoDescription={C.HERO.subtitle}
      path="/learn/marriage/marriage-timing"
      eyebrow="Marriage & Compatibility · Part 6 of 7"
      title={C.HERO.title}
    ...
    >
      ...
      <div className="mt-10">
        <RelatedArticles variant="next" title="Marriage & Compatibility path" items={[
          { title: 'Reading Two Charts Together: Synastry', href: '/learn/marriage/synastry', comingSoon: false, description: 'Continue the path.' },
        ]} />
      </div>
    </KnowledgeLayout>
  )
