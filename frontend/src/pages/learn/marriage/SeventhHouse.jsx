...
import * as C from '../../../config/marriage7thHouseContent'
...
export default function SeventhHouseGuide() {
  const { isAuthenticated } = useAuth()
  const sections = Object.entries(C).filter(([k]) => !['HERO','QUICK_FACTS','FAQ','CTA'].includes(k))
  
  return (
    <KnowledgeLayout
      seoTitle="The 7th House — Marriage & Compatibility Path"
      seoDescription={C.HERO.subtitle}
      path="/learn/marriage/7th-house"
      eyebrow="Marriage & Compatibility · Part 1 of 7"
      title={C.HERO.title}
    ...
    >
      ...
      <div className="mt-10">
        <RelatedArticles variant="next" title="Marriage & Compatibility path" items={[
          { title: 'Venus and Jupiter: The Natural Significators', href: '/learn/marriage/venus-jupiter', comingSoon: false, description: 'Continue the path.' },
        ]} />
      </div>
    </KnowledgeLayout>
  )
