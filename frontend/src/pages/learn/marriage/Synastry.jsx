...
import * as C from '../../../config/marriageSynastryContent'
...
export default function SynastryGuide() {
  const { isAuthenticated } = useAuth()
  const sections = Object.entries(C).filter(([k]) => !['HERO','QUICK_FACTS','FAQ','CTA'].includes(k))
  
  return (
    <KnowledgeLayout
      seoTitle="Synastry — Marriage & Compatibility Path"
      seoDescription={C.HERO.subtitle}
      path="/learn/marriage/synastry"
      eyebrow="Marriage & Compatibility · Part 7 of 7"
      title={C.HERO.title}
    ...
    >
      ...
      <div className="mt-10">
        <RelatedArticles variant="next" title="Marriage & Compatibility path" items={[
          { title: 'Marriage & Compatibility — Full Path', href: '/learn/paths/marriage-compatibility', comingSoon: false, description: 'Continue the path.' },
        ]} />
      </div>
    </KnowledgeLayout>
  )
