...
import * as C from '../../../config/marriageVenusJupiterContent'
...
export default function VenusJupiterGuide() {
  const { isAuthenticated } = useAuth()
  const sections = Object.entries(C).filter(([k]) => !['HERO','QUICK_FACTS','FAQ','CTA'].includes(k))
  
  return (
    <KnowledgeLayout
      seoTitle="Venus and Jupiter — Marriage & Compatibility Path"
      seoDescription={C.HERO.subtitle}
      path="/learn/marriage/venus-jupiter"
      eyebrow="Marriage & Compatibility · Part 2 of 7"
      title={C.HERO.title}
      subtitle={C.HERO.subtitle}
      meta={<LearningMetadata estimatedReadTime={7} difficulty="intermediate" category="Marriage" lastUpdated="2026-07-18" />}
      breadcrumbItems={[
        { label: 'Home', to: isAuthenticated ? '/home' : '/' },
        { label: 'Learn', to: '/learn' },
        { label: 'Marriage & Compatibility', to: '/learn/paths/marriage-compatibility' },
        { label: 'Venus and Jupiter' },
      ]}
    ...
    >
      ...
      <div className="mt-10">
        <RelatedArticles variant="next" title="Marriage & Compatibility path" items={[
          { title: 'The D9 Navamsa Chart: The Marriage Chart', href: '/learn/marriage/d9-navamsa', comingSoon: false, description: 'Continue the path.' },
        ]} />
      </div>
    </KnowledgeLayout>
  )
