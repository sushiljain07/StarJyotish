// frontend/src/pages/learn/zodiac/Aquarius.jsx
//
// /learn/zodiac/aquarius — Vedic Astrology guide for Kumbha (Aquarius).
// Mirrors Taurus.jsx exactly in structure and component usage.
// All copy lives in config/aquariusContent.js; this file only composes layout.
...
export default function AquariusGuide() {
  const { i18n } = useTranslation()
  const isHindi = i18n.language?.startsWith('hi')
  const guide = getGuide('zodiac-aquarius')
  const learningPathSteps = getLearningPathSteps('zodiac')
  const { prev, next } = getSignNavigation('zodiac-aquarius')
  const { isAuthenticated } = useAuth()

  return (
    <KnowledgeLayout
      seoTitle="Kumbha (Aquarius) in Vedic Astrology — A Complete Guide"
      seoDescription="Understand Kumbha (Aquarius) in Vedic astrology — the archetype, symbolism, strengths, growth areas, and how this sign fits into a complete birth chart."
      path="/learn/zodiac/aquarius"
      ...
      title={AQUARIUS_HERO.title}
      subtitle={AQUARIUS_HERO.subtitle}
      meta={
        <LearningMetadata
      ...
      cta={
        <CTA
          eyebrow={AQUARIUS_CTA.eyebrow}
          title={AQUARIUS_CTA.title}
          description={AQUARIUS_CTA.description}
          buttonLabel={AQUARIUS_CTA.buttonLabel}
          to="/generate"
          variant="full"
        />
      }
    >
      ...
      <Reveal className="flex flex-col sm:flex-row items-start gap-6 mb-2">
        <div className="shrink-0 w-16 h-16 rounded-2xl bg-primary-light flex items-center justify-center text-primary-dark">
          <ZodiacSignIcon id="aquarius" className="w-9 h-9" />
        </div>
        <div className="flex-1">
          <QuickFacts facts={AQUARIUS_QUICK_FACTS} columns={2} />
        </div>
      </Reveal>
      ...
      <section className="mt-12" aria-labelledby="spirit-heading">
        <Reveal>
          <p className="text-primary-dark text-xs font-bold tracking-widest uppercase mb-2">{SPIRIT_OF_KUMBHA.eyebrow}</p>
          <h2 id="spirit-heading" className="font-serif font-semibold text-2xl sm:text-3xl text-ink leading-snug mb-6">
            {SPIRIT_OF_KUMBHA.title}
          </h2>
          <ArticleParagraphs paragraphs={SPIRIT_OF_KUMBHA.paragraphs} />
        </Reveal>
      </section>
      ...
      <section className="mt-14" aria-labelledby="symbol-heading">
        <Reveal>
          <p className="text-primary-dark text-xs font-bold tracking-widest uppercase mb-2">{SYMBOL_OF_WATERBEAR.eyebrow}</p>
          <h2 id="symbol-heading" className="font-serif font-semibold text-2xl sm:text-3xl text-ink leading-snug mb-6">
            {SYMBOL_OF_WATERBEAR.title}
          </h2>
          <ArticleParagraphs paragraphs={SYMBOL_OF_WATERBEAR.paragraphs} />
        </Reveal>
      </section>
      ...
      <section className="mt-14" aria-labelledby="faq-heading">
        ...
        <FAQ items={AQUARIUS_FAQ} title={null} />
      </section>
