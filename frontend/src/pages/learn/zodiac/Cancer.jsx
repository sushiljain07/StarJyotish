// frontend/src/pages/learn/zodiac/Cancer.jsx
//
// /learn/zodiac/cancer — Vedic Astrology guide for Karka (Cancer).
// Mirrors Taurus.jsx exactly in structure and component usage.
// All copy lives in config/cancerContent.js; this file only composes layout.
...
export default function CancerGuide() {
  const { i18n } = useTranslation()
  const isHindi = i18n.language?.startsWith('hi')
  const guide = getGuide('zodiac-cancer')
  const learningPathSteps = getLearningPathSteps('zodiac')
  const { prev, next } = getSignNavigation('zodiac-cancer')
  const { isAuthenticated } = useAuth()

  return (
    <KnowledgeLayout
      seoTitle="Karka (Cancer) in Vedic Astrology — A Complete Guide"
      seoDescription="Understand Karka (Cancer) in Vedic astrology — the archetype, symbolism, strengths, growth areas, and how this sign fits into a complete birth chart."
      path="/learn/zodiac/cancer"
      ...
      title={CANCER_HERO.title}
      subtitle={CANCER_HERO.subtitle}
      meta={
        <LearningMetadata
      ...
      cta={
        <CTA
          eyebrow={CANCER_CTA.eyebrow}
          title={CANCER_CTA.title}
          description={CANCER_CTA.description}
          buttonLabel={CANCER_CTA.buttonLabel}
          to="/generate"
          variant="full"
        />
      }
    >
      ...
      <Reveal className="flex flex-col sm:flex-row items-start gap-6 mb-2">
        <div className="shrink-0 w-16 h-16 rounded-2xl bg-primary-light flex items-center justify-center text-primary-dark">
          <ZodiacSignIcon id="cancer" className="w-9 h-9" />
        </div>
        <div className="flex-1">
          <QuickFacts facts={CANCER_QUICK_FACTS} columns={2} />
        </div>
      </Reveal>
      ...
      <section className="mt-12" aria-labelledby="spirit-heading">
        <Reveal>
          <p className="text-primary-dark text-xs font-bold tracking-widest uppercase mb-2">{SPIRIT_OF_KARKA.eyebrow}</p>
          <h2 id="spirit-heading" className="font-serif font-semibold text-2xl sm:text-3xl text-ink leading-snug mb-6">
            {SPIRIT_OF_KARKA.title}
          </h2>
          <ArticleParagraphs paragraphs={SPIRIT_OF_KARKA.paragraphs} />
        </Reveal>
      </section>
      ...
      <section className="mt-14" aria-labelledby="symbol-heading">
        <Reveal>
          <p className="text-primary-dark text-xs font-bold tracking-widest uppercase mb-2">{SYMBOL_OF_CRAB.eyebrow}</p>
          <h2 id="symbol-heading" className="font-serif font-semibold text-2xl sm:text-3xl text-ink leading-snug mb-6">
            {SYMBOL_OF_CRAB.title}
          </h2>
          <ArticleParagraphs paragraphs={SYMBOL_OF_CRAB.paragraphs} />
        </Reveal>
      </section>
      ...
      <section className="mt-14" aria-labelledby="faq-heading">
        ...
        <FAQ items={CANCER_FAQ} title={null} />
      </section>
