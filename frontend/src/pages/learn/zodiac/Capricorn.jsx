// frontend/src/pages/learn/zodiac/Capricorn.jsx
//
// /learn/zodiac/capricorn — Vedic Astrology guide for Makara (Capricorn).
// Mirrors Taurus.jsx exactly in structure and component usage.
// All copy lives in config/capricornContent.js; this file only composes layout.
...
export default function CapricornGuide() {
  const { i18n } = useTranslation()
  const isHindi = i18n.language?.startsWith('hi')
  const guide = getGuide('zodiac-capricorn')
  const learningPathSteps = getLearningPathSteps('zodiac')
  const { prev, next } = getSignNavigation('zodiac-capricorn')
  const { isAuthenticated } = useAuth()

  return (
    <KnowledgeLayout
      seoTitle="Makara (Capricorn) in Vedic Astrology — A Complete Guide"
      seoDescription="Understand Makara (Capricorn) in Vedic astrology — the archetype, symbolism, strengths, growth areas, and how this sign fits into a complete birth chart."
      path="/learn/zodiac/capricorn"
      ...
      title={CAPRICORN_HERO.title}
      subtitle={CAPRICORN_HERO.subtitle}
      meta={
        <LearningMetadata
      ...
      cta={
        <CTA
          eyebrow={CAPRICORN_CTA.eyebrow}
          title={CAPRICORN_CTA.title}
          description={CAPRICORN_CTA.description}
          buttonLabel={CAPRICORN_CTA.buttonLabel}
          to="/generate"
          variant="full"
        />
      }
    >
      ...
      <Reveal className="flex flex-col sm:flex-row items-start gap-6 mb-2">
        <div className="shrink-0 w-16 h-16 rounded-2xl bg-primary-light flex items-center justify-center text-primary-dark">
          <ZodiacSignIcon id="capricorn" className="w-9 h-9" />
        </div>
        <div className="flex-1">
          <QuickFacts facts={CAPRICORN_QUICK_FACTS} columns={2} />
        </div>
      </Reveal>
      ...
      <section className="mt-12" aria-labelledby="spirit-heading">
        <Reveal>
          <p className="text-primary-dark text-xs font-bold tracking-widest uppercase mb-2">{SPIRIT_OF_MAKARA.eyebrow}</p>
          <h2 id="spirit-heading" className="font-serif font-semibold text-2xl sm:text-3xl text-ink leading-snug mb-6">
            {SPIRIT_OF_MAKARA.title}
          </h2>
          <ArticleParagraphs paragraphs={SPIRIT_OF_MAKARA.paragraphs} />
        </Reveal>
      </section>
      ...
      <section className="mt-14" aria-labelledby="symbol-heading">
        <Reveal>
          <p className="text-primary-dark text-xs font-bold tracking-widest uppercase mb-2">{SYMBOL_OF_SEAGOAT.eyebrow}</p>
          <h2 id="symbol-heading" className="font-serif font-semibold text-2xl sm:text-3xl text-ink leading-snug mb-6">
            {SYMBOL_OF_SEAGOAT.title}
          </h2>
          <ArticleParagraphs paragraphs={SYMBOL_OF_SEAGOAT.paragraphs} />
        </Reveal>
      </section>
      ...
      <section className="mt-14" aria-labelledby="faq-heading">
        ...
        <FAQ items={CAPRICORN_FAQ} title={null} />
      </section>
