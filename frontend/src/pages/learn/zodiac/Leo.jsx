// frontend/src/pages/learn/zodiac/Leo.jsx
//
// /learn/zodiac/leo — Vedic Astrology guide for Simha (Leo).
// Mirrors Taurus.jsx exactly in structure and component usage.
// All copy lives in config/leoContent.js; this file only composes layout.
...
export default function LeoGuide() {
  ...
  const guide = getGuide('zodiac-leo')
  const learningPathSteps = getLearningPathSteps('zodiac')
  const { prev, next } = getSignNavigation('zodiac-leo')
  const { isAuthenticated } = useAuth()

  return (
    <KnowledgeLayout
      seoTitle="Simha (Leo) in Vedic Astrology — A Complete Guide"
      seoDescription="Understand Simha (Leo) in Vedic astrology — the archetype, symbolism, strengths, growth areas, and how this sign fits into a complete birth chart."
      path="/learn/zodiac/leo"
      ...
      title={LEO_HERO.title}
      subtitle={LEO_HERO.subtitle}
      meta={
        <LearningMetadata
      ...
      cta={
        <CTA
          eyebrow={LEO_CTA.eyebrow}
          title={LEO_CTA.title}
          description={LEO_CTA.description}
          buttonLabel={LEO_CTA.buttonLabel}
          to="/generate"
          variant="full"
        />
      }
    >
      ...
      <Reveal className="flex flex-col sm:flex-row items-start gap-6 mb-2">
        <div className="shrink-0 w-16 h-16 rounded-2xl bg-primary-light flex items-center justify-center text-primary-dark">
          <ZodiacSignIcon id="leo" className="w-9 h-9" />
        </div>
        <div className="flex-1">
          <QuickFacts facts={LEO_QUICK_FACTS} columns={2} />
        </div>
      </Reveal>
      ...
      <section className="mt-12" aria-labelledby="spirit-heading">
        <Reveal>
          <p className="text-primary-dark text-xs font-bold tracking-widest uppercase mb-2">{SPIRIT_OF_SIMHA.eyebrow}</p>
          <h2 id="spirit-heading" className="font-serif font-semibold text-2xl sm:text-3xl text-ink leading-snug mb-6">
            {SPIRIT_OF_SIMHA.title}
          </h2>
          <ArticleParagraphs paragraphs={SPIRIT_OF_SIMHA.paragraphs} />
        </Reveal>
      </section>
      ...
      <section className="mt-14" aria-labelledby="symbol-heading">
        <Reveal>
          <p className="text-primary-dark text-xs font-bold tracking-widest uppercase mb-2">{SYMBOL_OF_LION.eyebrow}</p>
          <h2 id="symbol-heading" className="font-serif font-semibold text-2xl sm:text-3xl text-ink leading-snug mb-6">
            {SYMBOL_OF_LION.title}
          </h2>
          <ArticleParagraphs paragraphs={SYMBOL_OF_LION.paragraphs} />
        </Reveal>
      </section>
