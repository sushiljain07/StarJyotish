// frontend/src/pages/learn/zodiac/Libra.jsx
//
// /learn/zodiac/libra — Vedic Astrology guide for Tula (Libra).
// Mirrors Taurus.jsx exactly in structure and component usage.
// All copy lives in config/libraContent.js; this file only composes layout.
...
export default function LibraGuide() {
  const { i18n } = useTranslation()
  const isHindi = i18n.language?.startsWith('hi')
  const guide = getGuide('zodiac-libra')
  const learningPathSteps = getLearningPathSteps('zodiac')
  const { prev, next } = getSignNavigation('zodiac-libra')
  const { isAuthenticated } = useAuth()

  return (
    <KnowledgeLayout
      seoTitle="Tula (Libra) in Vedic Astrology — A Complete Guide"
      seoDescription="Understand Tula (Libra) in Vedic astrology — the archetype, symbolism, strengths, growth areas, and how this sign fits into a complete birth chart."
      path="/learn/zodiac/libra"
      ...
      title={LIBRA_HERO.title}
      subtitle={LIBRA_HERO.subtitle}
      meta={
        <LearningMetadata
      ...
      cta={
        <CTA
          eyebrow={LIBRA_CTA.eyebrow}
          title={LIBRA_CTA.title}
          description={LIBRA_CTA.description}
          buttonLabel={LIBRA_CTA.buttonLabel}
          to="/generate"
          variant="full"
        />
      }
    >
      ...
      <Reveal className="flex flex-col sm:flex-row items-start gap-6 mb-2">
        <div className="shrink-0 w-16 h-16 rounded-2xl bg-primary-light flex items-center justify-center text-primary-dark">
          <ZodiacSignIcon id="libra" className="w-9 h-9" />
        </div>
        <div className="flex-1">
          <QuickFacts facts={LIBRA_QUICK_FACTS} columns={2} />
        </div>
      </Reveal>
      ...
      <section className="mt-12" aria-labelledby="spirit-heading">
        <Reveal>
          <p className="text-primary-dark text-xs font-bold tracking-widest uppercase mb-2">{SPIRIT_OF_TULA.eyebrow}</p>
          <h2 id="spirit-heading" className="font-serif font-semibold text-2xl sm:text-3xl text-ink leading-snug mb-6">
            {SPIRIT_OF_TULA.title}
          </h2>
          <ArticleParagraphs paragraphs={SPIRIT_OF_TULA.paragraphs} />
        </Reveal>
      </section>
      ...
      <section className="mt-14" aria-labelledby="symbol-heading">
        <Reveal>
          <p className="text-primary-dark text-xs font-bold tracking-widest uppercase mb-2">{SYMBOL_OF_SCALES.eyebrow}</p>
          <h2 id="symbol-heading" className="font-serif font-semibold text-2xl sm:text-3xl text-ink leading-snug mb-6">
            {SYMBOL_OF_SCALES.title}
          </h2>
          <ArticleParagraphs paragraphs={SYMBOL_OF_SCALES.paragraphs} />
        </Reveal>
      </section>
      ...
      <section className="mt-14" aria-labelledby="faq-heading">
        ...
        <FAQ items={LIBRA_FAQ} title={null} />
      </section>
