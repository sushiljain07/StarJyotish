// frontend/src/pages/learn/zodiac/Pisces.jsx
//
// /learn/zodiac/pisces — Vedic Astrology guide for Meena (Pisces).
// Mirrors Taurus.jsx exactly in structure and component usage.
// All copy lives in config/piscesContent.js; this file only composes layout.
...
export default function PiscesGuide() {
  const { i18n } = useTranslation()
  const isHindi = i18n.language?.startsWith('hi')
  const guide = getGuide('zodiac-pisces')
  const learningPathSteps = getLearningPathSteps('zodiac')
  const { prev, next } = getSignNavigation('zodiac-pisces')
  const { isAuthenticated } = useAuth()

  return (
    <KnowledgeLayout
      seoTitle="Meena (Pisces) in Vedic Astrology — A Complete Guide"
      seoDescription="Understand Meena (Pisces) in Vedic astrology — the archetype, symbolism, strengths, growth areas, and how this sign fits into a complete birth chart."
      path="/learn/zodiac/pisces"
      ...
      title={PISCES_HERO.title}
      subtitle={PISCES_HERO.subtitle}
      meta={
        <LearningMetadata
      ...
      cta={
        <CTA
          eyebrow={PISCES_CTA.eyebrow}
          title={PISCES_CTA.title}
          description={PISCES_CTA.description}
          buttonLabel={PISCES_CTA.buttonLabel}
          to="/generate"
          variant="full"
        />
      }
    >
      ...
      <Reveal className="flex flex-col sm:flex-row items-start gap-6 mb-2">
        <div className="shrink-0 w-16 h-16 rounded-2xl bg-primary-light flex items-center justify-center text-primary-dark">
          <ZodiacSignIcon id="pisces" className="w-9 h-9" />
        </div>
        <div className="flex-1">
          <QuickFacts facts={PISCES_QUICK_FACTS} columns={2} />
        </div>
      </Reveal>
      ...
      <section className="mt-12" aria-labelledby="spirit-heading">
        <Reveal>
          <p className="text-primary-dark text-xs font-bold tracking-widest uppercase mb-2">{SPIRIT_OF_MEENA.eyebrow}</p>
          <h2 id="spirit-heading" className="font-serif font-semibold text-2xl sm:text-3xl text-ink leading-snug mb-6">
            {SPIRIT_OF_MEENA.title}
          </h2>
          <ArticleParagraphs paragraphs={SPIRIT_OF_MEENA.paragraphs} />
        </Reveal>
      </section>
      ...
      <section className="mt-14" aria-labelledby="symbol-heading">
        <Reveal>
          <p className="text-primary-dark text-xs font-bold tracking-widest uppercase mb-2">{SYMBOL_OF_FISH.eyebrow}</p>
          <h2 id="symbol-heading" className="font-serif font-semibold text-2xl sm:text-3xl text-ink leading-snug mb-6">
            {SYMBOL_OF_FISH.title}
          </h2>
          <ArticleParagraphs paragraphs={SYMBOL_OF_FISH.paragraphs} />
        </Reveal>
      </section>
      ...
      <section className="mt-14" aria-labelledby="faq-heading">
        ...
        <FAQ items={PISCES_FAQ} title={null} />
      </section>
