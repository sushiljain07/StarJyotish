// frontend/src/pages/learn/zodiac/Sagittarius.jsx
//
// /learn/zodiac/sagittarius — Vedic Astrology guide for Dhanu (Sagittarius).
// Mirrors Taurus.jsx exactly in structure and component usage.
// All copy lives in config/sagittariusContent.js; this file only composes layout.
...
import {
  ...
  SAGITTARIUS_FAQ,
  SAGITTARIUS_CTA,
} from '../../../config/sagittariusContent'
...
export default function SagittariusGuide() {
  const { i18n } = useTranslation()
  const isHindi = i18n.language?.startsWith('hi')
  const guide = getGuide('zodiac-sagittarius')
  const learningPathSteps = getLearningPathSteps('zodiac')
  const { prev, next } = getSignNavigation('zodiac-sagittarius')
  const { isAuthenticated } = useAuth()

  return (
    <KnowledgeLayout
      seoTitle="Dhanu (Sagittarius) in Vedic Astrology — A Complete Guide"
      seoDescription="Understand Dhanu (Sagittarius) in Vedic astrology — the archetype, symbolism, strengths, growth areas, and how this sign fits into a complete birth chart."
      path="/learn/zodiac/sagittarius"
      ...
      title={SAGITTARIUS_HERO.title}
      subtitle={SAGITTARIUS_HERO.subtitle}
      meta={
        <LearningMetadata
          estimatedReadTime={guide?.estimatedReadTime ?? 13}
      ...
      cta={
        <CTA
          eyebrow={SAGITTARIUS_CTA.eyebrow}
          title={SAGITTARIUS_CTA.title}
          description={SAGITTARIUS_CTA.description}
          buttonLabel={SAGITTARIUS_CTA.buttonLabel}
          to="/generate"
          variant="full"
        />
      }
    >
      ...
      <Reveal className="flex flex-col sm:flex-row items-start gap-6 mb-2">
        <div className="shrink-0 w-16 h-16 rounded-2xl bg-primary-light flex items-center justify-center text-primary-dark">
          <ZodiacSignIcon id="sagittarius" className="w-9 h-9" />
        </div>
        <div className="flex-1">
          <QuickFacts facts={SAGITTARIUS_QUICK_FACTS} columns={2} />
        </div>
      </Reveal>
      ...
      <section className="mt-12" aria-labelledby="spirit-heading">
        <Reveal>
          <p className="text-primary-dark text-xs font-bold tracking-widest uppercase mb-2">{SPIRIT_OF_DHANU.eyebrow}</p>
          <h2 id="spirit-heading" className="font-serif font-semibold text-2xl sm:text-3xl text-ink leading-snug mb-6">
            {SPIRIT_OF_DHANU.title}
          </h2>
          <ArticleParagraphs paragraphs={SPIRIT_OF_DHANU.paragraphs} />
        </Reveal>
      </section>
      ...
      <section className="mt-14" aria-labelledby="symbol-heading">
        <Reveal>
          <p className="text-primary-dark text-xs font-bold tracking-widest uppercase mb-2">{SYMBOL_OF_ARCHER.eyebrow}</p>
          <h2 id="symbol-heading" className="font-serif font-semibold text-2xl sm:text-3xl text-ink leading-snug mb-6">
            {SYMBOL_OF_ARCHER.title}
          </h2>
          <ArticleParagraphs paragraphs={SYMBOL_OF_ARCHER.paragraphs} />
        </Reveal>
      </section>
      ...
      <section className="mt-14" aria-labelledby="faq-heading">
        ...
        <FAQ items={SAGITTARIUS_FAQ} title={null} />
      </section>
