// frontend/src/pages/learn/zodiac/Scorpio.jsx
//
// /learn/zodiac/scorpio — Vedic Astrology guide for Vrishchika (Scorpio).
// Mirrors Taurus.jsx exactly in structure and component usage.
// All copy lives in config/scorpioContent.js; this file only composes layout.
...
export default function ScorpioGuide() {
  const { i18n } = useTranslation()
  const isHindi = i18n.language?.startsWith('hi')
  const guide = getGuide('zodiac-scorpio')
  const learningPathSteps = getLearningPathSteps('zodiac')
  const { prev, next } = getSignNavigation('zodiac-scorpio')
  const { isAuthenticated } = useAuth()

  return (
    <KnowledgeLayout
      seoTitle="Vrishchika (Scorpio) in Vedic Astrology — A Complete Guide"
      seoDescription="Understand Vrishchika (Scorpio) in Vedic astrology — the archetype, symbolism, strengths, growth areas, and how this sign fits into a complete birth chart."
      path="/learn/zodiac/scorpio"
      ...
      title={SCORPIO_HERO.title}
      subtitle={SCORPIO_HERO.subtitle}
      meta={
        <LearningMetadata
          estimatedReadTime={guide?.estimatedReadTime ?? 14}
      ...
      cta={
        <CTA
          eyebrow={SCORPIO_CTA.eyebrow}
          title={SCORPIO_CTA.title}
          description={SCORPIO_CTA.description}
          buttonLabel={SCORPIO_CTA.buttonLabel}
          to="/generate"
          variant="full"
        />
      }
    >
      ...
      <Reveal className="flex flex-col sm:flex-row items-start gap-6 mb-2">
        <div className="shrink-0 w-16 h-16 rounded-2xl bg-primary-light flex items-center justify-center text-primary-dark">
          <ZodiacSignIcon id="scorpio" className="w-9 h-9" />
        </div>
        <div className="flex-1">
          <QuickFacts facts={SCORPIO_QUICK_FACTS} columns={2} />
        </div>
      </Reveal>
      ...
      <section className="mt-12" aria-labelledby="spirit-heading">
        <Reveal>
          <p className="text-primary-dark text-xs font-bold tracking-widest uppercase mb-2">{SPIRIT_OF_VRISHCHIKA.eyebrow}</p>
          <h2 id="spirit-heading" className="font-serif font-semibold text-2xl sm:text-3xl text-ink leading-snug mb-6">
            {SPIRIT_OF_VRISHCHIKA.title}
          </h2>
          <ArticleParagraphs paragraphs={SPIRIT_OF_VRISHCHIKA.paragraphs} />
        </Reveal>
      </section>
      ...
      <section className="mt-14" aria-labelledby="symbol-heading">
        <Reveal>
          <p className="text-primary-dark text-xs font-bold tracking-widest uppercase mb-2">{SYMBOL_OF_SCORPION.eyebrow}</p>
          <h2 id="symbol-heading" className="font-serif font-semibold text-2xl sm:text-3xl text-ink leading-snug mb-6">
            {SYMBOL_OF_SCORPION.title}
          </h2>
          <ArticleParagraphs paragraphs={SYMBOL_OF_SCORPION.paragraphs} />
        </Reveal>
      </section>
      ...
      <section className="mt-14" aria-labelledby="faq-heading">
        ...
        <FAQ items={SCORPIO_FAQ} title={null} />
      </section>
