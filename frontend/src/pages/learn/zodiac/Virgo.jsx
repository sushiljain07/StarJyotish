// frontend/src/pages/learn/zodiac/Virgo.jsx
//
// /learn/zodiac/virgo — Vedic Astrology guide for Kanya (Virgo).
// Mirrors Taurus.jsx exactly in structure and component usage.
// All copy lives in config/virgoContent.js; this file only composes layout.
...
export default function VirgoGuide() {
  const { i18n } = useTranslation()
  const isHindi = i18n.language?.startsWith('hi')
  const guide = getGuide('zodiac-virgo')
  const learningPathSteps = getLearningPathSteps('zodiac')
  const { prev, next } = getSignNavigation('zodiac-virgo')
  const { isAuthenticated } = useAuth()

  return (
    <KnowledgeLayout
      seoTitle="Kanya (Virgo) in Vedic Astrology — A Complete Guide"
      seoDescription="Understand Kanya (Virgo) in Vedic astrology — the archetype, symbolism, strengths, growth areas, and how this sign fits into a complete birth chart."
      path="/learn/zodiac/virgo"
      ...
      title={VIRGO_HERO.title}
      subtitle={VIRGO_HERO.subtitle}
      meta={
        <LearningMetadata
      ...
      cta={
        <CTA
          eyebrow={VIRGO_CTA.eyebrow}
          title={VIRGO_CTA.title}
          description={VIRGO_CTA.description}
          buttonLabel={VIRGO_CTA.buttonLabel}
          to="/generate"
          variant="full"
        />
      }
    >
      ...
      <Reveal className="flex flex-col sm:flex-row items-start gap-6 mb-2">
        <div className="shrink-0 w-16 h-16 rounded-2xl bg-primary-light flex items-center justify-center text-primary-dark">
          <ZodiacSignIcon id="virgo" className="w-9 h-9" />
        </div>
        <div className="flex-1">
          <QuickFacts facts={VIRGO_QUICK_FACTS} columns={2} />
        </div>
      </Reveal>
      ...
      <section className="mt-12" aria-labelledby="spirit-heading">
        <Reveal>
          <p className="text-primary-dark text-xs font-bold tracking-widest uppercase mb-2">{SPIRIT_OF_KANYA.eyebrow}</p>
          <h2 id="spirit-heading" className="font-serif font-semibold text-2xl sm:text-3xl text-ink leading-snug mb-6">
            {SPIRIT_OF_KANYA.title}
          </h2>
          <ArticleParagraphs paragraphs={SPIRIT_OF_KANYA.paragraphs} />
        </Reveal>
      </section>
      ...
      <section className="mt-14" aria-labelledby="symbol-heading">
        <Reveal>
          <p className="text-primary-dark text-xs font-bold tracking-widest uppercase mb-2">{SYMBOL_OF_MAIDEN.eyebrow}</p>
          <h2 id="symbol-heading" className="font-serif font-semibold text-2xl sm:text-3xl text-ink leading-snug mb-6">
            {SYMBOL_OF_MAIDEN.title}
          </h2>
          <ArticleParagraphs paragraphs={SYMBOL_OF_MAIDEN.paragraphs} />
        </Reveal>
      </section>
      ...
      <section className="mt-14" aria-labelledby="faq-heading">
        ...
        <FAQ items={VIRGO_FAQ} title={null} />
      </section>
